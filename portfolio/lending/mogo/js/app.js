...
window.addEventListener("scroll", function (scrollPos, introH) {
  ...
  var logosH =
    document.getElementById("logos").scrollHeight -
    120 +
    document.getElementById("logos").getBoundingClientRect().top +
    window.scrollY;
  ...
  if (scrollPos < introH) {
    ...
    scrollPos <
      document.getElementById("blog").getBoundingClientRect().top +
        window.scrollY -
        100 +
        document.getElementById("blog").scrollHeight
  ) {
...
function activeLink(elSelector, activeClass) {
  ...
  links.forEach((thisLink) => {
    thisLink.addEventListener("click", (e) => {
      ...
      coordY =
        document
          .querySelector(thisLink.getAttribute("href"))
